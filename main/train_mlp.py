...
from Utils import utils, edge_rw, homophily_distribution
# import optuna
...
def arg_parse():
    ...
    parser.add_argument('--mlp_conv_dim', type=int, help='hidden dimension for classification layer', default=64)
...
def train_one_epoch(mlp_model, gnn_model, src_data, tgt_data, args,
                    mlp_opt, gnn_opt, mlp_scheduler, gnn_scheduler, epoch):
    ...
    mlp_embed, mlp_logits = mlp_model(src_data)
    mask_src =  src_data.source_training_mask
    label_src = src_data.y[mask_src]
    mlp_pred_src = mlp_logits[mask_src]

    mlp_loss_src = utils.CE_loss(mlp_pred_src, label_src)
    mlp_loss_src.backward()
    mlp_opt.step()
    mlp_scheduler.step()
    ...
    gnn_embed_list, gnn_logits = gnn_model(src_data)
    gnn_pred_src = gnn_logits[mask_src]

    gnn_loss_src = utils.CE_loss(gnn_pred_src, label_src)
    gnn_loss_src.backward()
    gnn_opt.step()
    gnn_scheduler.step()
...
def train(source_dataset, target_dataset, args, seed):
    ...
    gnn_model = models.GNNWithMLPClassifier(input_dim, output_dim, args).to(device)
    mlp_scheduler, mlp_opt = utils.build_optimizer(args, mlp_model.parameters())
    gnn_scheduler, gnn_opt = utils.build_optimizer(args, gnn_model.parameters())


    best_valid_score_mlp = 0
    best_valid_score_gnn = 0
    best_epoch_mlp = 0
    best_epoch_gnn = 0


    mlp_valid_accs, gnn_valid_accs = [], []
    ...
    for epoch in range(args.epochs):
        ...
        train_one_epoch(
            ...
            args,
            mlp_opt, gnn_opt,
            mlp_scheduler, gnn_scheduler,
            epoch
        )

        print(f"\nEpoch {epoch + 1} ---------------------")
        report_mlp, report_gnn,_,_,_,_ = evaluate(source_dataset, target_dataset, mlp_model, gnn_model)


        valid_score_mlp = report_mlp['acc_tgt_valid'] if args.valid_data != "src" else report_mlp['acc_src_valid']
        valid_score_gnn = report_gnn['acc_tgt_valid'] if args.valid_data != "src" else report_gnn['acc_src_valid']

        print(f"MLP valid acc: {valid_score_mlp:.4f} | GNN valid acc: {valid_score_gnn:.4f}")


        if valid_score_mlp > best_valid_score_mlp:
            best_valid_score_mlp = valid_score_mlp
            best_epoch_mlp = epoch
            torch.save(mlp_model.state_dict(), os.path.join(path, f'best_mlp_model_seed{seed}.pt'))


        if valid_score_gnn > best_valid_score_gnn:
            best_valid_score_gnn = valid_score_gnn
            best_epoch_gnn = epoch
            torch.save(gnn_model.state_dict(), os.path.join(path, f'best_gnn_model_seed{seed}.pt'))


        mlp_valid_accs.append(valid_score_mlp)
        gnn_valid_accs.append(valid_score_gnn)


    mlp_model.load_state_dict(torch.load(os.path.join(path, f'best_mlp_model_seed{seed}.pt')))
    gnn_model.load_state_dict(torch.load(os.path.join(path, f'best_gnn_model_seed{seed}.pt')))
    mlp_model = mlp_model.to(device)
    gnn_model = gnn_model.to(device)

    final_report_mlp, final_report_gnn,mlp_embed_src,mlp_embed_tgt,GNN_embed_src,GNN_embed_tgt = evaluate(source_dataset, target_dataset, mlp_model, gnn_model)

    return final_report_mlp



def evaluate(source_dataset, target_dataset, mlp_model, gnn_model):
    # ===== 评估 MLP 模型 ===== #
    loss_src_train, mlp_embed_src_train, acc_src_train, auc_src_train = evaluate_dataset(
        source_dataset, mlp_model, "src_train")
    loss_src_valid, mlp_embed_src_valid, acc_src_valid, auc_src_valid = evaluate_dataset(
        source_dataset, mlp_model, "src_valid")
    loss_src_test, mlp_embed_src_test, acc_src_test, auc_src_test = evaluate_dataset(
        source_dataset, mlp_model, "src_test")
    loss_tgt_valid, mlp_embed_tgt_valid, acc_tgt_valid, auc_tgt_valid = evaluate_dataset(
        target_dataset, mlp_model, "tgt_valid")
    loss_tgt_test, mlp_embed_tgt_test, acc_tgt_test, auc_tgt_test = evaluate_dataset(
        target_dataset, mlp_model, "tgt_test")
    ...
    report_mlp = {
        ...
        'acc_tgt_valid': acc_tgt_valid, 'auc_tgt_valid': auc_tgt_valid,
        'acc_tgt_test': acc_tgt_test, 'auc_tgt_test': auc_tgt_test,
        'default': acc_tgt_valid
    }

    # ===== 评估 GNN 模型 ===== #
    loss_src_train, GNN_embed_src_train, acc_src_train, auc_src_train = evaluate_dataset(
        source_dataset, gnn_model, "src_train")
    loss_src_valid, GNN_embed_src_valid, acc_src_valid, auc_src_valid = evaluate_dataset(
        source_dataset, gnn_model, "src_valid")
    loss_src_test, GNN_embed_src_test, acc_src_test, auc_src_test = evaluate_dataset(
        source_dataset, gnn_model, "src_test")
    loss_tgt_valid, GNN_embed_tgt_valid, acc_tgt_valid, auc_tgt_valid = evaluate_dataset(
        target_dataset, gnn_model, "tgt_valid")
    loss_tgt_test, GNN_embed_tgt_test, acc_tgt_test, auc_tgt_test = evaluate_dataset(
        target_dataset, gnn_model, "tgt_test")
    ...
    return report_mlp, report_gnn, mlp_embed_src_test,mlp_embed_tgt_test,GNN_embed_src_test,GNN_embed_tgt_test
...
def main():
    ...
    args.mlp_conv_dim = optimized_params['mlp_conv_dim']
    args.mlp_cls_dim = optimized_params['mlp_cls_dim']
